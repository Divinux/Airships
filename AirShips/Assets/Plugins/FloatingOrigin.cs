...
//
// There should only be one FloatingOrigin component in your scene.
//
// Make sure that you assign the FloatingOrigin.singleton.focus transform field so the script knows where to recenter the world to.
...
namespace twoloop
{
    ...
    {
        public static FloatingOrigin singleton;
        ...
        public static Offset LocalOffset
        {
            get
            {
                ...
                {
                    Debug.Log(
                        "FloatingOrigin: get LocalOffset was called when there was no FloatingOrigin initialized.");
                    return new Offset();
                }
            }
            private set
...
#if MIRROR_43_0_OR_NEWER
                    if (singleton.networkFou && singleton.networkFou.isServer)
                    {
                        singleton.networkFou.SetHostOffset(value);
                    }
#endif
                }
                else
        ...
        /// </summary>
        /// <param name="remoteOffset">The remote client's FloatingOrigin.LocalOffset</param>
        ...
        private void OnValidate()
        {
            ...
            if (networkFou == null)
            {
                networkFou = FindObjectOfType<NetworkFOU>();
            }
#endif
        }
        ...
        private void Awake()
        {
            ...
            {
                Debug.LogWarning("There can only be one FloatingOrigin in the scene.");
                Destroy(this);
            }
...
#if MIRROR_43_0_OR_NEWER
            // Condition for resetting static value when there is no singleton
            NetworkFOU.hostOffset = new Offset();
#endif
        }
